...
def phi(t, t_p=None, Q_sq=0, ff='+',  **kwargs):
    if ff=='0':
        f1 = np.sqrt(eta*t_plus*t_minus/(2*np.pi))/(t_cut-t_0)
        f2 = (1+z(t))/((1-z(t))**(5/2))
...
#    COV_input=np.array(gp['stat_cov'])
#import pickle
#pickle.dump([known_ts, known_ffs, COV_input], open(f'BstoK_Data_{nplus}x{nzero}.p','wb'))
import pickle
[known_ts, known_ffs, COV_input] = pickle.load(open(f'BstoK_Data_{nplus}x{nzero}.p','rb'))
known_ts = np.hstack((known_ts, known_ts))
