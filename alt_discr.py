...
method = input('1. beta^2 + alpha*gamma\n'+
               '2. det(M11)*get(G)\n'+
               '3. appendix identity\n'+
               'method (1/2/3):')
while method not in ['1','2','3']:
    print('Err: method choice must be 1, 2, or 3')
    method = input('1. beta^2 + alpha*gamma\n'+
                   '2. det(M11)*get(G)\n'+
                   '3. appendix identity\n'+
                   'method (1/2/3):')
...
def alt_det_G(t_vec, **kwargs):
    f1 = 1/(np.sum([(1-(z(t)**2)) for t in t_vec]))
    f2 = 1
    N = len(t_vec)
    for j in range(1,N):
        for i in range(j):
            f2 = f2*(z(t_vec[i])-z(t_vec[j]))/(1-z(t_vec[i])*z(t_vec[j]))
    f2 = f2**2
    return f1*f2
...
def bounds(unknown_t, known_ts, known_ffs, X, prnt=False, **kwargs):
    ...
    if method=='1':
        ...
        disc = det(g)*det(M11(known_ts, known_ffs, X, **kwargs))
    elif method=='3':
        disc = alt_det_G(known_ts)*det(M11(known_ts, known_ffs, X, **kwargs))
...
def final_bounds(dist):
    ...
    for i in range(len(accepted_ts)):
        ...
        N = len(lows)
        rho = np.sum(np.outer(lows-f_lo, ups-f_lo))/(N-1)
